  ...
  <div>
    <el-card>
      ...
      <el-table class="mt-20" :data="tableData" border>
        ...
        <el-table-column align="center" label="操作" prop="action" fixed="right" width="280">
          <template slot-scope="scope">
            <HButton :permission="pBenefit.month.monthList" type="check" @click="handleCheckMonth(scope.row)">查看月报表</HButton>
            <HButton :permission="pBenefit.month.yearList" type="check" @click="handleCheckYear(scope.row)">查看年报表</HButton>
          </template>
        </el-table-column>
      </el-table>
      <pagination :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getList" />
    </el-card>
    <ActionCheckMonth ref="ActionCheckMonth" />
    <ActionCheckYear ref="ActionCheckYear" />
  </div>
</template>
...
import ActionCheckMonth from './components/ActionCheckMonth'
import ActionCheckYear from './components/ActionCheckYear'
import { mapGetters } from 'vuex'

export default {
  name: 'EquipmentPage',
  components: { ActionCheckMonth, ActionCheckYear },
  data() {
    return {
      listQuery: {
        page: 1,
  ...
  methods: {
    ...
    handleCheckMonth(data) {
      this.$refs.ActionCheckMonth.openDialog({ ...data })
    },
    handleCheckYear(data) {
      this.$refs.ActionCheckYear.openDialog({ ...data })
    }
  }
}
</script>
